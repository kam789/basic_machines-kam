...
basic_machines.plant_table           = {
    ...
    ["farming:cocoa_beans_raw"] = "farming:cocoa_1",
...
basic_machines.limit_inventory_table = { -- node name = {list of bad inventories to take from} OR node name = true to ban all inventories
    ...
    ["smartshop:shop"] = true,
    ["itemframes:frame"] = true,
    ["pipeworks:autocrafter"] = true,
    ["pipeworks:digiline_filter'"] = true,
    ["pipeworks:mese_tube_000000"] = true,
    ["pipeworks:mese_tube"] = true,
    ["pipeworks:mese_filter"] = true,
    ["xdecor:enchantment_table"] = true,
    ["pipeworks:filter"] = true,
    ["fancy_vend:player_vendor"] = true,
    ["3d_armor_stand:locked_armor_stand"] = true,
    ["3d_armor_stand:armor_stand"] = true,
}
...
local get_mover_form = function(pos, player)
    ...
    x0 = meta:get_int("x0"); y0 = meta:get_int("y0"); z0 = meta:get_int("z0"); x1 = meta:get_int("x1"); y1 = meta
        :get_int("y1"); z1 = meta:get_int("z1"); x2 = meta:get_int("x2"); y2 = meta:get_int("y2"); z2 = meta:get_int(
        "z2");

    machines.pos1[player:get_player_name()] = { x = pos.x + x0, y = pos.y + y0, z = pos.z + z0 }; machines.mark_pos1(
        player:get_player_name()) -- mark pos1
    machines.pos11[player:get_player_name()] = { x = pos.x + x1, y = pos.y + y1, z = pos.z + z1 }; machines.mark_pos11(
        player:get_player_name()) -- mark pos11
    machines.pos2[player:get_player_name()] = { x = pos.x + x2, y = pos.y + y2, z = pos.z + z2 }; machines.mark_pos2(
        player:get_player_name()) -- mark pos2
    ...
    if seltab == 1 then -- MODE --
        local mode_description = {
            ...
            ["object"] =
            "make TELEPORTER/ELEVATOR. This will move any object inside sphere (with center source1 and radius defined by source2) to target position. For ELEVATOR teleport points need to be placed exactly in same coordinate line with mover and you need to upgrade with 1 diamondblock for every 100 height difference. ",
            ["inventory"] =
            "This will move items from inventory of any block at source position to any inventory of block at target position",
            ["transport"] =
            "This will move all blocks at source area to new area starting at target position. This mode preserves all inventories and other metadata",
        };
        ...
        form = "size[8,8.25]" .. -- width, height
            ...
            "list[nodemeta:" ..
            pos.x .. ',' .. pos.y .. ',' .. pos.z .. ";upgrade;5,3.4;1,1;]" ..
            "label[4,3;UPGRADE LVL " .. upgrade .. "]" ..
        ...
        if mode_string == "inventory" then
            inventory_list1 = "label[4.5,0.25;source inventory] dropdown[4.5,0.75;1.5,1;inv1;" ..
                inv_list1 .. ";" .. inv1 .. "]"
            inventory_list2 = "label[4.5,3.;target inventory] dropdown[4.5,3.5;1.5,1;inv2;" ..
                inv_list2 .. ";" .. inv2 .. "]"
        else
        ...
        form = "size[8,8.25]" .. -- width, height
            ...
            "field[0.25,1.;1,1;x0;source1;" .. x0 ..
            "] field[1.25,1.;1,1;y0;;" .. y0 .. "] field[2.25,1.;1,1;z0;;" .. z0 .. "]" ..
            "image[3,0.75;1,1;machines_pos1.png]" ..
            inventory_list1 ..
            "field[0.25,2;1,1;x1;source2;" .. x1 .. "] field[1.25,2;1,1;y1;;" .. y1 ..
            "] field[2.25,2;1,1;z1;;" .. z1 .. "]" ..
            "image[3,1.75;1,1;machines_pos11.png]" ..
            "label[0.,2.75;" .. minetest.colorize("red", "TARGET POSITION - mover will move to here") .. "]" ..
            "field[0.25,3.75;1,1;x2;Target;" ..
            x2 .. "] field[1.25,3.75;1,1;y2;;" .. y2 .. "] field[2.25,3.75;1,1;z2;;" .. z2 .. "]" ..
...
minetest.register_node("basic_machines:mover", {
    ...
    allow_metadata_inventory_put = function(pos, listname, index, stack, player)
        ...
        if listname == "upgrade" then
            ...
            if stack:get_name() == upgrade_name then
                ...
                local form = get_mover_form(pos, player)
                minetest.show_formspec(player:get_player_name(), "basic_machines:mover_" .. minetest.pos_to_string(pos),
                    form)
            end
        end
    ...
    effector = {
        action_on = function(pos, node, ttl)
            ...
            local x1 = meta:get_int("x1") - x0 + 1; local y1 = meta:get_int("y1") - y0 + 1; local z1 = meta:get_int("z1") -
                z0 + 1;                                                                                                 -- get dimensions
            ...
            if node1.name == "default:chest_locked" or mode == "inventory" then
                fuel_cost = basic_machines.hardness
                    [prefer] or 1
            end;
            ...
            if mode == "object" then
                ...
                if (x2 == 0 and y2 == 0) or (x2 == 0 and z2 == 0) or (y2 == 0 and z2 == 0) then -- check if elevator mode
                    local requirement = math.floor((math.abs(pos2.x - pos.x) + math.abs(pos2.y - pos.y) + math.abs(pos2.z - pos.z)) /
                        100) + 1;
                    if upgrade - 1 < requirement then
                        meta:set_string("infotext",
                            "MOVER: Elevator error. Need at least " ..
                            requirement .. " diamond block(s) in upgrade (1 for every 100 distance). ");
                        return;
                    end
                    fuel_cost = 0
                end
            ...
            if fuel < fuel_cost then
                meta:set_string("infotext",
                    "Mover block. Energy " ..
                    fuel .. ", needed energy " .. fuel_cost .. ". Put nonempty battery next to mover.");
                return
            end
            ...
            if mode == "object" then -- teleport objects and return
                ...
                if times ~= 0 then
                    ...
                    local vv = math.sqrt(velocityv.x * velocityv.x + velocityv.y * velocityv.y + velocityv.z *
                        velocityv.z);
                    local velocitys = 0;
                        ...
                        if not basic_machines.no_teleport_table[detected_obj] then -- object on no teleport list
                            if times > 0 then
                                ...
                                if finalmove then -- dont move objects like balls to destination after delay
                                    minetest.after(times,
                                        function()
                                            if obj then
                                                obj:setvelocity({ x = 0, y = 0, z = 0 }); obj:moveto(pos2, false)
                                            end
                                        end);
                                end
                            else
                ...
                if teleport_any then
                    ...
                    if finalsound then
                        minetest.sound_play("tng_transporter1",
                            { pos = pos2, gain = 1.0, max_hear_distance = 8, })
                    end
                end
            ...
            if mode == "inventory" then
                ...
                local function starts_with(str, sti) return str:sub(1, #sti) == sti end
                local t68 = starts_with(node1.name, "pipeworks:mese_tube")
                ...
                if limit_inventory or t68 then
                    if t68 or limit_inventory == true or limit_inventory[invName1] then -- forbidden to take from this inventory
                        return
                    end
                end
...
local function use_keypad(pos, ttl, again) -- position, time to live ( how many times can signal travel before vanishing to prevent infinite recursion ), do we want to activate again
    ...
    if minetest.is_protected(pos, name) then
        meta:set_string("infotext", "Protection fail. reset."); meta:set_int("count", 0); return
    end
    ...
    if text ~= "" then                    -- TEXT MODE; set text on target
        ...
        elseif bit == 36 then -- text starts with $, play sound
            ...
            if not i then
                ...
                minetest.sound_play(string.sub(text, 1, i - 1), {
                    pos = pos,
                    gain = 1.0,
...
minetest.register_node("basic_machines:keypad", {
    ...
    on_rightclick = function(pos, node, player, itemstack, pointed_thing)
        ...
        local form =
            ...
            "field[0.25,3.5;1,1;x0;target;" ..
            x0 .. "] field[1.25,3.5;1,1;y0;;" .. y0 .. "] field[2.25,3.5;1,1;z0;;" .. z0 .. "]" ..
...
minetest.register_node("basic_machines:detector", {
    ...
    on_rightclick = function(pos, node, player, itemstack, pointed_thing)
        ...
        machines.pos1[player:get_player_name()] = { x = pos.x + x0, y = pos.y + y0, z = pos.z + z0 }; machines.mark_pos1(
            player:get_player_name())             -- mark pos1
        machines.pos11[player:get_player_name()] = { x = pos.x + x1, y = pos.y + y1, z = pos.z + z1 }; machines
            .mark_pos11(player:get_player_name()) -- mark pos11
        machines.pos2[player:get_player_name()] = { x = pos.x + x2, y = pos.y + y2, z = pos.z + z2 }; machines.mark_pos2(
            player:get_player_name())             -- mark pos2
        ...
        local form      =
            ...
            "field[0.25,0.5;1,1;x0;source1;" ..
            x0 .. "] field[1.25,0.5;1,1;y0;;" .. y0 .. "] field[2.25,0.5;1,1;z0;;" .. z0 .. "]" ..
            "dropdown[3,0.25;1,1;op; ,AND,OR;" .. op .. "]" ..
            "field[0.25,1.5;1,1;x1;source2;" ..
            x1 .. "] field[1.25,1.5;1,1;y1;;" .. y1 .. "] field[2.25,1.5;1,1;z1;;" .. z1 .. "]" ..
            "field[0.25,2.5;1,1;x2;target;" ..
            x2 .. "] field[1.25,2.5;1,1;y2;;" .. y2 .. "] field[2.25,2.5;1,1;z2;;" .. z2 .. "]" ..
    ...
    effector = {
        action_on = function(pos, node, ttl)
            ...
            elseif NOT == -1 then
                trigger = not trigger -- NEGATION
            elseif NOT == -2 and trigger then
                return                -- ONLY FALSE
            elseif NOT == 0 then
                return                -- do nothing
            elseif NOT == 2 and not trigger then
                return                -- ONLY TRUE
...
minetest.register_node("basic_machines:clockgen", {
    ...
    after_place_node = function(pos, placer)
        ...
        meta:set_string("infotext",
            "clock generator (owned by " .. owner .. "): place machine to be activated on top of generator");
    end
})
    ...
    if view == 0 then
        ...
        for i = 1, n do
            form = form ..
                ...
                (0.5 + (i - 1) * 0.75) ..
                ";1,1;z" .. i ..
                ";;" .. p[i].z .. "] field [ 3.25," .. (0.5 + (i - 1) * 0.75) .. ";1,1;active" .. i ..
                ";;" .. active[i] .. "]"
            form = form ..
                "button[4.," ..
                (0.25 + (i - 1) * 0.75) ..
                ...
                "]" ..
                "button_exit[5.25," ..
                (0.25 + (i - 1) * 0.75) .. ";1,1;SET" .. i .. ";SET]" ..
                "button[6.25," .. (0.25 + (i - 1) * 0.75) .. ";1,1;X" .. i .. ";X]"
        end

        form = form .. "button[4.0," .. (0.25 + (n) * 0.75) .. ";1,1;ADD;ADD]" ..
            "button[5.25," .. (0.25 + (n) * 0.75) .. ";1,1;view;view]" ..
            "button_exit[3.," ..
            (0.25 + (n) * 0.75) .. ";1,1;OK;OK]" .. "field[0.25," .. (0.5 + (n) * 0.75) ..
            ";1,1;delay;delay;" .. delay .. "]";
        ...
        local form =
            "size[7," .. (0.75 + (n) * 0.75) .. "]" .. -- width, height
            "label[0,-0.25;" ..
            minetest.colorize("lawngreen", "target name, MODE -2=only OFF, -1=NOT input/0/1=input, 2 = only ON") .. "]";
        for i = 1, n do
            local tname = minetest.get_node({ x = p[i].x + pos.x, y = p[i].y + pos.y, z = p[i].z + pos.z }).name;
            local ti = string.find(tname, ":") or 0; tname = p[i].x ..
                " " .. p[i].y .. " " .. p[i].z .. " " .. string.sub(tname, ti + 1);

            form = form ..
                "field[0.25," ..
                (0.5 + (i - 1) * 0.75) ..
                ";3,1;text;;" .. tname .. "] field [ 3.25," .. (0.5 + (i - 1) * 0.75) ..
...
minetest.register_node("basic_machines:distributor", {
    ...
    after_place_node = function(pos, placer)
        ...
        meta:set_string("owner", placer:get_player_name()); meta:set_int("public", 0);
        for i = 1, 10 do
            meta:set_int("x" .. i, 0); meta:set_int("y" .. i, 1); meta:set_int("z" .. i, 0); meta:set_int("active" .. i,
                1)                  -- target i
        end
    ...
    effector = {
        action_on = function(pos, node, ttl)
            ...
            local activate = function()
                ...
                for i = 1, n do
                    posf[i] = {
                        x = meta:get_int("x" .. i) + pos.x,
                        y = meta:get_int("y" .. i) + pos.y,
                        z = meta:get_int(
                            "z" .. i) + pos.z
                    };
                    active[i] = meta:get_int("active" .. i);
    ...
    on_rightclick = function(pos, node, player, itemstack, pointed_thing)
        ...
        if form then
            minetest.show_formspec(player:get_player_name(),
                "basic_machines:distributor_" .. minetest.pos_to_string(pos), form)
        end
    end,
}
)
...
minetest.register_node("basic_machines:light_on", {
    ...
    after_place_node = function(pos, placer)
        ...
        local list_name  = "nodemeta:" .. pos.x .. ',' .. pos.y .. ',' .. pos.z
        local deactivate = meta:get_int("deactivate");
        local form       = "size[2,2] field[0.25,0.5;2,1;deactivate;deactivate after ;" ..
            deactivate .. "]" .. "button_exit[0.,1;1,1;OK;OK]";
...
minetest.register_on_punchnode(function(pos, node, puncher, pointed_thing)
    ...
    if punchset[name].node == "basic_machines:mover" then -- mover code, not first punch
        ...
        if punchset[name].state == 3 then
            ...
            if (punchset[name].pos.x == pos.x and punchset[name].pos.z == pos.z) or
                ...
                local ecost = math.abs(punchset[name].pos.y - pos.y) + math.abs(punchset[name].pos.x - pos.x) +
                    math.abs(punchset[name].pos.z - pos.z)
                    ...
                    if meta:get_string("mode") == "object" then -- only if object mode
                        ...
                        if upgrade < requirement then
                            minetest.chat_send_player(name,
                                "MOVER: Error while trying to make elevator. Need at least " ..
                                requirement .. " diamond block(s) in upgrade (1 for every 100 distance). ");
                            punchset[name].state = 0; return
                        else
                            elevator_mode = true;
    ...
    if node.name == "basic_machines:detector" then -- detector init code
        ...
        if minetest.is_protected(pos, name) then
            minetest.chat_send_player(name, "DETECTOR: You must be able to build to set up detector.")
            return
        end
        if punchset[name].state == 0 then
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
            ...
            if seltab == 2 then -- POSITIONS
                ...
                if meta:get_int("x0") ~= x0 or meta:get_int("y0") ~= y0 or meta:get_int("z0") ~= z0 or
                    ...
                    if not privs.privs and (math.abs(x1) > max_range or math.abs(y1) > max_range or math.abs(z1) > max_range or math.abs(x2) > max_range or math.abs(y2) > max_range or math.abs(z2) > max_range) then
                        minetest.chat_send_player(name,
                            "#mover: all coordinates must be between " ..
                            -max_range .. " and " .. max_range .. ". For increased range set up positions by punching"); return
                    end
                end
                ...
                --notification
                meta:set_string("infotext",
                    "Mover block. Set up with source coordinates " ..
                    x0 ..
                    "," ..
                    y0 ..
                    "," ..
                    ...
                    " and target coord " ..
                    x2 .. "," .. y2 ..
                    "," .. z2 .. ". Put charged battery next to it and start it with keypad/mese signal.");
            else -- MODE 1
                if fields.mode then
    ...
    if string.sub(formname, 0, string.len(fname)) == fname then
        ...
        if fields.OK == "OK" then
            ...
            if not privs.privs and (math.abs(x0) > max_range or math.abs(y0) > max_range or math.abs(z0) > max_range) then
                minetest.chat_send_player(name,
                    "#keypad: all coordinates must be between " .. -max_range .. " and " .. max_range); return
            end
            meta:set_int("x0", x0); meta:set_int("y0", y0); meta:set_int("z0", z0);

            if fields.pass then
                if fields.pass ~= "" and string.len(fields.pass) <= 16 then -- dont replace password with hash which is longer - 27 chars
                    pass = minetest.get_password_hash(pos.x, pass .. pos.y); pass = minetest.get_password_hash(pos.y,
                        pass .. pos.z);
            ...
            if fields.text then
                ...
                if string.find(fields.text, "!") then
                    minetest.log("action",
                        string.format("%s set up keypad for message display at %s", name, minetest.pos_to_string(pos)))
                end
            end
    ...
    if string.sub(formname, 0, string.len(fname)) == fname then
        ...
        if fields.OK == "OK" then
            ...
            pass = minetest.get_password_hash(pos.x, pass .. pos.y); pass = minetest.get_password_hash(pos.y, pass ..
                pos.z);
    ...
    if string.sub(formname, 0, string.len(fname)) == fname then
        ...
        if fields.help == "help" then
            local text =
                "SETUP: right click or punch and follow chat instructions. With detector you can detect nodes, objects, players, or items inside inventories." ..
        ...
        if fields.OK == "OK" then
            ...
            if not privs.privs and (math.abs(x0) > max_range or math.abs(y0) > max_range or math.abs(z0) > max_range or math.abs(x1) > max_range or math.abs(y1) > max_range or math.abs(z1) > max_range) then
                minetest.chat_send_player(name,
                    "#detector: all coordinates must be between " .. -max_range .. " and " .. max_range); return
            end
    ...
    if string.sub(formname, 0, string.len(fname)) == fname then
        ...
        if fields.OK == "OK" then
            ...
            for i = 1, n do
                ...
                if (not (privs.privs) and math.abs(posf[i].x) > 2 * max_range or math.abs(posf[i].y) > 2 * max_range or math.abs(posf[i].z) > 2 * max_range) then
                    minetest.chat_send_player(name,
                        "#distributor: all coordinates must be between " .. -2 * max_range .. " and " .. 2 * max_range);
                    return
                end

                if posmode then
                    meta:set_int("x" .. i, posf[i].x); meta:set_int("y" .. i, posf[i].y); meta:set_int("z" .. i,
                        posf[i].z);
                end
        ...
        if fields["view"] then -- change view mode
            ...
            local form = get_distributor_form(pos, player)
            minetest.show_formspec(player:get_player_name(), "basic_machines:distributor_" .. minetest.pos_to_string(pos),
                form)
            return
        end
